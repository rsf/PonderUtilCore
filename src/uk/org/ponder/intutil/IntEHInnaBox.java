...
/**
 * The class <code>IntEHInnaBox</code>implements an extendible hash table of
...
public class IntEHInnaBox {
  public static final int KNUTH_GOLDEN_CONSTANT = (int) 2654435761L;
  public static int hash(int smallint) {
    return smallint * KNUTH_GOLDEN_CONSTANT;
  }
  ...
  private void checkexpand() {
    if ((double) (filled + 1) / storage.length > MAXIMUM_LOAD) {
      ...
      System.out
          .println("*******Load factor exceeded: *******: current_mask now "
              + uk.org.ponder.byteutil.ByteWrap.intToHex(current_mask) + " "
              + current_bits);
   ...
   * @param hash The hash value required. @param offset In the case where many
   * items are stored with identical hash keys, the number of the item with that
   * hash key that is required. @return The required value, or <code>INVALID_VALID</code>
  ...
  /*
   * Enters the specified value into the table with the specified hash key,
   * unless that value is already stored. @param hash The hash key for the
   * value. @param value The value that is to be stored. @return <code>true</code>
   * if the value was successfully stored, <code>false</code> if it was
   * already present and the table was left unaltered.
   */
  public boolean put(int hash, int value) {
    ...
    int initindex = (hash & current_mask);
    // System.out.println("Putting "+ hash+" value " + value +" at "+
    // initindex);
