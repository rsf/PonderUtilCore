 ...
 * hidden state of a field <code>nestedpath</code> which corresponds to the
 * EL location for which the data alterations currently being processed are
 * based at. TargettedMessage objects received to this list via the 
 * {@link #addMessage(TargettedMessage)} methods will have their <code>targetid</code>
 * field, if set, interpreted as an EL relative to this hidden path and rebased.
...
//at least, which are fairly clean in of themselves.
   ...
   * field will be interpreted as an EL path adjusted to take into account of the current
   * value of <code>nestedPath</code> 
   */
  ...
  /** Does the current state of the TML represent an error? This is determined
   * by checking for any individual message which is at the 
